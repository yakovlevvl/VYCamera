...
let frameworkBundle = Bundle(for: VYCamera.self)
...
extension UIImage {
    ...
    convenience init?(named: String) {
        self.init(named: named, in: frameworkBundle, compatibleWith: nil)
    }
}

extension UIButton {
    
    func setImage(_ image: UIImage, state: UIControl.State = .normal) {
        setImage(image, for: state)
    }
    
    func setImage(_ named: String, for state: UIControl.State = .normal) {
        setImage(UIImage(named: named, in: frameworkBundle, compatibleWith: nil), for: state)
    }
}
